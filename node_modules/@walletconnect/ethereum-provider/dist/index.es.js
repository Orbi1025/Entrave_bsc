...
import {
  getAccountsFromNamespaces as O,
  getSdkError as x,
  isValidArray as y,
...
const $ = "wc",
  ...
  D = async () => {
    const { createAppKit: s } = await import("@reown/appkit/core");
    return s;
  };
...
function G(s) {
  ...
  if (!e && !o && !i && !(l != null && l.length) && !(r != null && r.length))
  ...
  return { required: t.length ? c : void 0, optional: e.length ? h : void 0 };
}
class b {
  constructor() {
    p(this, "events", new j()),
      p(this, "namespace", "eip155"),
      p(this, "accounts", []),
      p(this, "signer"),
      p(this, "chainId", 1),
      p(this, "modal"),
      p(this, "rpc"),
      p(this, "STORAGE_KEY", q),
      p(this, "on", (t, e) => (this.events.on(t, e), this)),
      p(this, "once", (t, e) => (this.events.once(t, e), this)),
      p(
        this,
        "removeListener",
        (t, e) => (this.events.removeListener(t, e), this)
      ),
      p(this, "off", (t, e) => (this.events.off(t, e), this)),
      p(this, "parseAccount", (t) =>
  ...
  async connect(t) {
    ...
    try {
      const a = await new Promise(async (u, c) => {
        ...
        await this.signer
          .connect(
            w(
              v(
                { namespaces: v({}, n && { [this.namespace]: n }) },
                i && { optionalNamespaces: { [this.namespace]: i } }
              ),
              { pairingTopic: t?.pairingTopic, scopedProperties: m }
            )
          )
          .then((h) => {
            u(h);
      ...
      this.setChainIds(this.rpc.chains.length ? this.rpc.chains : o),
        this.setAccounts(o),
        this.events.emit("connect", { chainId: E(this.chainId) });
    } catch (a) {
            ...
            (l = this.modal) == null ||
              l.subscribeState((r) => {
                !r.open &&
                  !this.signer.session &&
                  (this.signer.abortPairingAttempt(),
                  u(new Error("Connection request reset. Please try again.")));
              })),
            await this.signer
              .authenticate(w(v({}, t), { chains: this.rpc.chains }), e)
              .then((r) => {
                o(r);
              })
              .catch((r) => {
                var m;
                (m = this.modal) == null ||
                  m.showErrorMessage("Unable to connect"),
                  u(new Error(r.message));
              });
        }),
        a = i.session;
      if (a) {
  ...
  registerEventListeners() {
    this.signer.on("session_event", (t) => {
      ...
      this.signer.on("accountsChanged", (t) => {
        (this.accounts = this.parseAccounts(t)),
          this.events.emit("accountsChanged", this.accounts);
      }),
      this.signer.on("chainChanged", (t) => {
        const e = parseInt(t);
        (this.chainId = e),
          this.events.emit("chainChanged", E(this.chainId)),
          this.persist();
      }),
    ...
    if (
      ...
      try {
        const n = await D(),
          ...
          a = i(
            w(v({}, this.rpc.qrModalOptions), {
              chains: [
                ...new Set([...this.rpc.chains, ...this.rpc.optionalChains]),
              ],
              metadata: this.rpc.metadata,
              projectId: this.rpc.projectId,
            })
        ...
        e = n(
          w(v({}, a), { universalProvider: this.signer, manualWCControl: !0 })
        );
      } catch (n) {
        throw (
          (console.warn(n),
  ...
  loadConnectOpts(t) {
    ...
    e &&
      ...
      n &&
        ...
        (this.rpc.optionalChains = n?.map((a) => this.formatChainId(a))),
        n.forEach((a) => {
          this.rpc.rpcMap[a] = i?.[a] || this.getRpcUrl(a);
        }));
  }
  getRpcUrl(t, e) {
    ...
    return (
      ...
      `${U}?chainId=eip155:${t}&projectId=${e || this.rpc.projectId}`
    );
  }
  async loadPersistedSession() {
    if (this.session)
      try {
        ...
        this.setChainIds(t ? [this.formatChainId(t)] : e?.accounts),
          this.setAccounts(e?.accounts);
      } catch (t) {
...
function st(s) {
  ...
  const l = c.find((m) => {
    ...
    r = {
      ...
      metadata: Z(R({}, s.metadata), {
        ...
        description:
          ((n = s.metadata) == null ? void 0 : n.description) ||
          "Connect to WalletConnect-compatible wallets",
        url:
          ((i = s.metadata) == null ? void 0 : i.url) ||
      ...
      showWallets: !0,
      featuredWalletIds:
        s.explorerRecommendedWalletIds === "NONE"
          ? []
          : Array.isArray(s.explorerRecommendedWalletIds)
          ? s.explorerRecommendedWalletIds
          : [],
      excludeWalletIds:
        s.explorerExcludedWalletIds === "ALL"
          ? []
          : Array.isArray(s.explorerExcludedWalletIds)
          ? s.explorerExcludedWalletIds
          : [],
      enableEIP6963: !1,
      enableInjected: !1,
      enableCoinbase: !0,
  ...
  if (
    ((o = s.mobileWallets) != null && o.length) ||
    ((u = s.desktopWallets) != null && u.length)
  ) {
    const m = [
        ...
        ...(s.desktopWallets || []).map((g) => ({
          id: g.id,
          name: g.name,
          links: { native: g.links.native, universal: g.links.universal },
        })),
      ],
...
function W(s) {
  return R({ formatters: void 0, fees: void 0, serializers: void 0 }, s);
}
...
export {
  ...
  P as OPTIONAL_EVENTS,
  A as OPTIONAL_METHODS,
  C as REQUIRED_EVENTS,
  f as REQUIRED_METHODS,
  b as default,
};
