...
import {
  ...
  useCallback as Q,
  useEffect as h,
  useImperativeHandle as fe,
  useMemo as me,
  useRef as D,
  useState as Z,
} from "react";
import { forwardRef as ce } from "react";
import { jsx as ue } from "react/jsx-runtime";
var ae = ({ as: n = "div", ...a }, s) => ue(n, { ...a, ref: s }),
  J = ce(ae);
import { useEffect as le, useState as de } from "react";
var K = "https://challenges.cloudflare.com/turnstile/v0/api.js",
  b = "cf-turnstile-script",
  U = "cf-turnstile",
  P = "onloadTurnstileCallback",
  M = (n) => !!document.getElementById(n),
  X = ({
    render: n = "explicit",
    onLoadCallbackName: a = P,
    scriptOptions: {
      nonce: s = "",
      defer: e = !0,
      async: m = !0,
    ...
    if (
      ((i.id = E),
      (i.src = `${K}?onload=${a}&render=${n}`),
      document.querySelector(`script[src="${i.src}"]`))
    )
      return;
    (i.defer = !!e),
      (i.async = !!m),
  ...
  f = {
    ...
    interactionOnly: { width: "fit-content", height: "auto", display: "flex" },
  };
function G(n) {
...
function z(n = b) {
  ...
  return (
    le(() => {
      ...
      return (
        m.observe(document, { childList: !0, subtree: !0 }),
        e(),
        () => {
          m.disconnect();
        }
...
var _ = "unloaded",
  ...
  Ee = pe((n, a) => {
      ...
      j = Q(
        () =>
          typeof c > "u"
            ...
            : e.execution === "execute"
            ? f.invisible
            : e.appearance === "interaction-only"
            ? f.interactionOnly
            : f[c],
        [e.execution, c, e.appearance]
      ),
      [ie, R] = Z(j()),
      u = D(null),
      [x, B] = Z(!1),
    ...
    let O = s?.id || b,
      A = z(O),
      V = s?.onLoadCallbackName || P,
      se = e.appearance || "always",
      C = me(
        () => ({
          sitekey: m,
          action: e.action,
          ...
          "before-interactive-callback": o
            ? E
            : (...t) => d.current.onBeforeInteractive?.(...t),
          "after-interactive-callback": o
            ? i
            : (...t) => d.current.onAfterInteractive?.(...t),
          "unsupported-callback": o
            ? I
            : (...t) => d.current.onUnsupported?.(...t),
      ...
      y = Q(() => typeof window < "u" && !!window.turnstile, []);
    return (
      h(
        function () {
          $ &&
            !x &&
            X({ onLoadCallbackName: V, scriptOptions: { ...s, id: O } });
        },
        [$, x, s, O]
      ),
      h(function () {
        ...
        function () {
          ...
          return (
            ...
            () => {
              (l = !0),
                r.current &&
                  (window.turnstile.remove(r.current), (L.current = !1));
            }
          );
        },
        [H, x, C]
          ...
          return {
            ...
            async getResponsePromise(l = 3e4, Y = 100) {
              return new Promise((F, N) => {
                let p,
                  q = async () => {
                    if (L.current && window.turnstile && r.current)
                      try {
                        let S = window.turnstile.getResponse(r.current);
                        return (
                          p && clearTimeout(p),
                          S ? F(S) : N(new Error("No response received"))
                        );
                      } catch (S) {
                        return (
                          p && clearTimeout(p),
                          console.warn("Failed to get response", S),
                          N(new Error("Failed to get response"))
                        );
                      }
                    p ||
                      (p = setTimeout(() => {
                      ...
                      await new Promise((S) => setTimeout(S, Y)),
                      await q();
                  };
                q();
              });
            ...
            render() {
              ...
              return (
                (r.current = l),
                r.current && v?.(r.current),
                e.execution !== "execute" && R(c ? f[c] : {}),
                l
              );
            },
            execute() {
            ...
            isExpired() {
              return !t?.isExpired || !r.current || !y()
                ? (console.warn("Turnstile has not been loaded"), !1)
                : t.isExpired(r.current);
            },
          };
        },
        [r, e.execution, c, C, u, y, x, v]
      ),
      h(() => {
      ...
      xe(J, { ref: u, as: ne, id: H, style: { ...ie, ...re }, ...oe })
    );
  });
