...
export function fromAbi(abi, name, options) {
  ...
  if (abiItems.length === 0) throw new NotFoundError({ name: name });
  ...
  if (!abiItem) throw new NotFoundError({ name: name });
  return {
    ...abiItem,
    ...(prepare ? { hash: getSignatureHash(abiItem) } : {}),
  };
  ...
  constructor(x, y) {
    super("Found ambiguous types in overloaded ABI Items.", {
      metaMessages: [
        ...
        `\`${x.type}\` in \`${internal.normalizeSignature(
          abitype.formatAbiItem(x.abiItem)
        )}\`, and`,
        `\`${y.type}\` in \`${internal.normalizeSignature(
          abitype.formatAbiItem(y.abiItem)
        )}\``,
        "",
...
export class InvalidSelectorSizeError extends Errors.BaseError {
  constructor({ data }) {
    super(
      `Selector size is invalid. Expected 4 bytes. Received ${Hex.size(
        data
      )} bytes ("${data}").`
    );
    Object.defineProperty(this, "name", {
